...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch { /* Older clients sent an empty body, which means add. */ }
  const result = action === 'remove'
    ? await db.prepare('UPDATE feedback SET likes = MAX(likes - 1, 0) WHERE id = ?').bind(id).run()
    : await db.prepare('UPDATE feedback SET likes = likes + 1 WHERE id = ?').bind(id).run();
  ...
  return Response.json({ likes: Number(row?.likes) || 0, liked: action === 'add' }, { headers: { 'Cache-Control': 'no-store' } });
}
