...
const PLAN_COLUMNS = 'id,title,destination,start_date,end_date,people,edit_policy,map_provider,view_mode,distance_base_ids_json,fuel_efficiency,fuel_price,stops_json,destinations_json,password_hash,password_salt,password_algo,edit_password_hash,edit_password_salt,edit_password_algo,edit_token_hash,created_at,updated_at,deleted_at,version,(password_hash IS NOT NULL) AS password_protected,(edit_password_hash IS NOT NULL) AS edit_password_protected';
const PLAN_INSERT = 'INSERT INTO plans (id,title,destination,start_date,end_date,people,edit_policy,map_provider,view_mode,distance_base_ids_json,fuel_efficiency,fuel_price,stops_json,destinations_json,password_hash,password_salt,password_algo,edit_password_hash,edit_password_salt,edit_password_algo,edit_token_hash,created_at,updated_at,deleted_at,version) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
...
export async function POST(request: Request, context: Context) {
  ...
  if (body.action === 'duplicate') {
    ...
    const copied = await insertCopy(db, row, { id, title: `${textValue(row.title)} 복제본`.slice(0, 160), destination: textValue(row.destination), startDate: textValue(row.start_date), endDate: textValue(row.end_date), people: Number(row.people) || 1, editPolicy: 'all', mapProvider: row.map_provider === 'google' ? 'google' : row.map_provider === 'osm' ? 'osm' : 'naver', viewMode: row.view_mode === 'distance' ? 'distance' : 'route', distanceBaseIds: textValue(row.distance_base_ids_json, '[]'), fuelEfficiency: Number(row.fuel_efficiency) || 12, fuelPrice: Number.isFinite(Number(row.fuel_price)) ? Number(row.fuel_price) : 1700, stops: textValue(row.stops_json, '[]'), destinations: textValue(row.destinations_json, '[]'), passwordHash: row.password_hash ? textValue(row.password_hash) : null, passwordSalt: row.password_salt ? textValue(row.password_salt) : null, passwordAlgo: row.password_algo === 'pbkdf2' ? 'pbkdf2' : 'sha256', editHash: null, editSalt: null, editAlgo: 'sha256', tokenHash: await sha256(editToken), now });
...
export async function PUT(request: Request, context: Context) {
  ...
  const values = [...(titleChanged ? [plan.title, plan.destination] : []), plan.startDate, plan.endDate, plan.people, plan.editPolicy, plan.mapProvider, plan.viewMode, JSON.stringify(plan.distanceBaseIds), plan.fuelEfficiency, plan.fuelPrice, JSON.stringify(plan.stops), JSON.stringify(plan.destinations), ...(passwordChanged ? [hash, salt, newPassword ? 'pbkdf2' : 'sha256'] : []), ...(editPasswordChanged ? [editHash, editSalt, editHash ? 'pbkdf2' : 'sha256'] : []), now, nextVersion, String(row.id), baseVersion];
  const guarded = await db.prepare(`UPDATE plans SET ${titleSql}start_date=?,end_date=?,people=?,edit_policy=?,map_provider=?,view_mode=?,distance_base_ids_json=?,fuel_efficiency=?,fuel_price=?,stops_json=?,destinations_json=?${passwordSql}${editPasswordSql},updated_at=?,version=? WHERE id=? AND version=?`).bind(...values).run();
  if (Number(guarded?.meta?.changes || 0) === 0) {
    ...
    const copied = await insertCopy(db, latest, { id, title: `${plan.title} - (동시편집본 ${conflictLabel})`.slice(0, 160), destination: plan.destination, startDate: plan.startDate, endDate: plan.endDate, people: plan.people, editPolicy: plan.editPolicy, mapProvider: plan.mapProvider, viewMode: plan.viewMode, distanceBaseIds: JSON.stringify(plan.distanceBaseIds), fuelEfficiency: plan.fuelEfficiency, fuelPrice: plan.fuelPrice, stops: JSON.stringify(plan.stops), destinations: JSON.stringify(plan.destinations), passwordHash: copiedPasswordHash, passwordSalt: copiedPasswordSalt, passwordAlgo: passwordChanged ? (newPassword ? 'pbkdf2' : 'sha256') : latest.password_algo === 'pbkdf2' ? 'pbkdf2' : 'sha256', editHash: copiedEditHash, editSalt: copiedEditSalt, editAlgo: editPasswordChanged ? (copiedEditHash ? 'pbkdf2' : 'sha256') : latest.edit_password_algo === 'pbkdf2' ? 'pbkdf2' : 'sha256', tokenHash: await sha256(editToken), now });
  ...
  const updated = { ...row, title: plan.title, destination: plan.destination, start_date: plan.startDate, end_date: plan.endDate, people: plan.people, edit_policy: plan.editPolicy, map_provider: plan.mapProvider, view_mode: plan.viewMode, distance_base_ids_json: JSON.stringify(plan.distanceBaseIds), fuel_efficiency: plan.fuelEfficiency, fuel_price: plan.fuelPrice, stops_json: JSON.stringify(plan.stops), destinations_json: JSON.stringify(plan.destinations), updated_at: now, version: nextVersion, ...(passwordChanged ? { password_hash: hash, password_salt: salt, password_algo: newPassword ? 'pbkdf2' : 'sha256', password_protected: Number(Boolean(hash)) } : {}), ...(editPasswordChanged ? { edit_password_hash: editHash, edit_password_salt: editSalt, edit_password_algo: editHash ? 'pbkdf2' : 'sha256', edit_password_protected: Number(Boolean(editHash)) } : {}) };
