...
const PLAN_COLUMNS = 'id,title,destination,start_date,end_date,people,edit_policy,map_provider,stops_json,destinations_json,password_hash,password_salt,password_algo,edit_password_hash,edit_password_salt,edit_password_algo,edit_token_hash,created_at,updated_at,deleted_at,version,(password_hash IS NOT NULL) AS password_protected,(edit_password_hash IS NOT NULL) AS edit_password_protected';
const PLAN_INSERT = 'INSERT INTO plans (id,title,destination,start_date,end_date,people,edit_policy,map_provider,stops_json,destinations_json,password_hash,password_salt,password_algo,edit_password_hash,edit_password_salt,edit_password_algo,edit_token_hash,created_at,updated_at,deleted_at,version) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
...
export async function PUT(request: Request, context: Context) {
  ...
  const values = [...(titleChanged ? [plan.title, plan.destination] : []), plan.startDate, plan.endDate, plan.people, plan.editPolicy, plan.mapProvider, JSON.stringify(plan.stops), JSON.stringify(plan.destinations), ...(passwordChanged ? [hash, salt, newPassword ? 'pbkdf2' : 'sha256'] : []), ...(editPasswordChanged ? [editHash, editSalt, editHash ? 'pbkdf2' : 'sha256'] : []), now, nextVersion, String(row.id), baseVersion];
  const guarded = await db.prepare(`UPDATE plans SET ${titleSql}start_date=?,end_date=?,people=?,edit_policy=?,map_provider=?,stops_json=?,destinations_json=?${passwordSql}${editPasswordSql},updated_at=?,version=? WHERE id=? AND version=?`).bind(...values).run();
