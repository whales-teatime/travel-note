...
const PLAN_SUMMARY_COLUMNS = 'p.id,p.title,p.destination,p.start_date,p.end_date,p.people,p.edit_policy,p.map_provider,p.view_mode,p.distance_base_ids_json,(p.password_hash IS NOT NULL) AS password_protected,(p.edit_password_hash IS NOT NULL) AS edit_password_protected,p.created_at,p.updated_at,p.deleted_at,p.version';
...
export async function GET(request: Request) {
  ...
  const result = search
    ...
    : await db.prepare(`SELECT id,title,destination,start_date,end_date,people,edit_policy,map_provider,view_mode,distance_base_ids_json,(password_hash IS NOT NULL) AS password_protected,(edit_password_hash IS NOT NULL) AS edit_password_protected,created_at,updated_at,deleted_at,version FROM plans WHERE ${deletedClause} ORDER BY updated_at DESC,id DESC LIMIT ? OFFSET ?`).bind(limit + 1, offset).all();
...
export async function POST(request: Request) {
  ...
  await db.prepare('INSERT INTO plans (id,title,destination,start_date,end_date,people,edit_policy,map_provider,view_mode,distance_base_ids_json,stops_json,destinations_json,password_hash,password_salt,edit_password_hash,edit_password_salt,edit_token_hash,created_at,updated_at,deleted_at,version,password_algo,edit_password_algo) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id, plan.title, plan.destination, plan.startDate, plan.endDate, plan.people, plan.editPolicy, plan.mapProvider, plan.viewMode, JSON.stringify(plan.distanceBaseIds), JSON.stringify(plan.stops), JSON.stringify(plan.destinations), hash, salt, editHash, editSalt, editTokenHash, now, now, null, 1, password ? 'pbkdf2' : 'sha256', editPassword ? 'pbkdf2' : 'sha256').run();
