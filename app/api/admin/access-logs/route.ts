...
type AccessLogRow = { id: unknown; created_at: unknown; path: unknown; status: unknown; country: unknown; visitor_hash: unknown };
...
export async function GET(request: Request) {
  ...
  try {
    const [last24h, last7d, unique24h, recent, popular] = await Promise.all([
      ...
      db.prepare('SELECT id,created_at,path,status,country,visitor_hash FROM access_logs ORDER BY created_at DESC,id DESC LIMIT 100').all<AccessLogRow>(),
    ...
    return Response.json({
      ...
      items: (recent.results || []).map(row => ({ id: textValue(row.id), createdAt: textValue(row.created_at), path: textValue(row.path, '/'), status: numberValue(row.status), country: textValue(row.country), visitor: textValue(row.visitor_hash).slice(0, 8) })),
