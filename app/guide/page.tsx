  ...
  return <main className="guide-page-shell">
    ...
    <Tabs defaultValue="pc" className="guide-page-tabs">
      ...
      <TabsContent value="pc" className="guide-tab-panel">
        <div className="guide-section-heading"><span>PC</span><div><h2>{text('마우스로 빠르게 계획하기', 'Plan quickly with your mouse')}</h2><p>{text('사진 속 화면은 실제 서비스에서 직접 조작해 촬영했습니다. 카드 더블클릭으로 지도에서 위치를 바로 찾을 수도 있어요.', 'These examples were recorded while using the live service. Double-click a card to find its pin on the map.')}</p></div></div>
        ...
        <div className="guide-detail-grid">
          ...
          <article><MapPin /><div><h3>{text('더블클릭으로 핀 찾기', 'Find a pin with a double-click')}</h3><p>{text('카드를 두 번 클릭하면 현재 날짜의 지도가 해당 장소로 이동하고 핀이 강조됩니다. 가까운 장소가 겹쳐 보여도 원하는 카드를 바로 찾을 수 있어요.', 'Double-click a card to move the current day’s map to that place and highlight its pin. It also helps when nearby places overlap.')}</p></div></article>
          <article><Compass /><div><h3>{text('해외 여행도 같은 방식으로', 'Plan overseas trips the same way')}</h3><p>{text('새 계획에서 해외로를 고르고 도시를 선택하면 세계 어디든 일정과 지도를 만들 수 있습니다. 해외 장소는 영문·현지어로 검색하고 후보에서 정확한 지점을 골라주세요.', 'Choose Explore the world when starting a plan, then select a city to plan anywhere. Search overseas places in English or the local language and choose the right result.')}</p></div></article>
        </div>
      </TabsContent>
      ...
      <TabsContent value="mobile" className="guide-tab-panel">
        ...
        <div className="guide-mobile-overview"><Image src="/guide/mobile-planner.webp" alt={text('모바일 여행 일정 화면 전체 모습', 'Full mobile travel planner screen')} width={430} height={932} unoptimized /><div><span className="guide-process-number">MOBILE TIPS</span><h3>{text('작은 화면에서는 이렇게 쓰세요', 'A few tips for small screens')}</h3><ul><li><GripVertical />{text('카드 순서는 오른쪽의 위·아래 화살표로 바꾸는 편이 빠릅니다.', 'Use the up and down arrows to reorder cards quickly.')}</li><li><Search />{text('검색창을 닫고 지도를 누르면 키보드도 함께 닫힙니다.', 'Close the search field and tap the map to hide the keyboard too.')}</li><li><MapPin />{text('임의 핀은 지도 위치를 정한 뒤 이름과 시간을 입력하면 됩니다.', 'Set a custom pin, then add its name and time.')}</li><li><Compass />{text('해외 여행은 해외로에서 도시를 고른 뒤 영문 장소명을 검색하세요.', 'For overseas trips, choose Explore the world, select a city, then search in English.')}</li><li><Save />{text('작성 중인 초안은 이 기기에 자동 저장되고, 완성한 계획은 계획 저장으로 서버에 보관합니다.', 'Drafts are saved on this device while you work. Use Save plan to keep the finished plan on the server.')}</li></ul></div></div>
      </TabsContent>
    </Tabs>

    <section className="guide-faq"><span className="guide-page-kicker"><Compass /> {text('알아두면 편해요', 'GOOD TO KNOW')}</span><div className="guide-faq-grid"><article><h3>{text('검색 결과가 애매해요', 'Search results look unclear')}</h3><p>{text('여행 일정에 입력한 도시가 우선 반영됩니다. 그래도 찾기 어렵다면 도시명 + 장소명으로 검색해 보세요.', 'Your trip city is used first. If a place is still hard to find, search with city name + place name.')}</p></article><article><h3>{text('해외 지도는 어떻게 쓰나요?', 'How do overseas maps work?')}</h3><p>{text('해외로에서 도시를 고르면 해당 도시가 기본 화면으로 열립니다. 도시·장소 이름은 영어 또는 현지어로 입력하고 후보를 선택하면 됩니다.', 'Choose a city from Explore the world and its map opens as the starting view. Enter a city or place in English or the local language, then choose a suggestion.')}</p></article><article><h3>{text('계획 목록에는 무엇이 보이나요?', 'What appears in the plan list?')}</h3><p>{text('비밀번호를 걸어도 여행 이름·여행지·날짜는 목록과 검색 결과에 표시될 수 있어요. 일정 내용과 메모는 열람 비밀번호로 보호됩니다.', 'Even with a password, the trip name, destination, and dates can appear in the list and search results. The itinerary and notes are protected by the viewing password.')}</p></article><article><h3>{text('친구도 수정할 수 있나요?', 'Can friends edit too?')}</h3><p>{text('편집 권한을 모두에게 열거나 편집 비밀번호를 따로 공유하면 함께 수정할 수 있어요. 작성자만으로 두면 작성자 토큰이 필요합니다.', 'Everyone can edit when you allow it, or when they have the separate editing password. Author-only plans require the author token.')}</p></article><article><h3>{text('실수로 계획을 삭제했어요', 'I deleted a plan by mistake')}</h3><p>{text('메인의 휴지통에서 7일 안에 복원할 수 있습니다. 7일이 지나면 자동으로 완전히 삭제됩니다.', 'Restore it from the Trash within 7 days. After that, it is permanently deleted.')}</p></article></div></section>
