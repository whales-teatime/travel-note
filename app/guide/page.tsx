...
export default function GuidePage() {
  ...
  return <main className="guide-page-shell">
    ...
    <Tabs defaultValue="pc" className="guide-page-tabs">
      ...
      <TabsContent value="pc" className="guide-tab-panel">
        ...
        <div className="guide-detail-grid">
          ...
          <article><LockKeyhole /><div><h3>{text('비밀번호로 내 계획 지키기', 'Keep your plan private')}</h3><p>{text('열람 비밀번호는 계획을 볼 수 있는 사람을 정하고, 편집 비밀번호는 수정할 수 있는 사람을 정합니다. 두 비밀번호를 따로 설정해 우리끼리만 여행 내용을 공유할 수 있어요.', 'A viewing password controls who can open a plan, while an editing password controls who can change it. Set them separately when you want to share the trip with just your group.')}</p></div></article>
        </div>
      </TabsContent>
    ...
    <section className="guide-faq"><span className="guide-page-kicker"><Compass /> {text('알아두면 편해요', 'GOOD TO KNOW')}</span><div className="guide-faq-grid"><article><h3>{text('검색 결과가 애매해요', 'Search results look unclear')}</h3><p>{text('여행 일정에 입력한 도시가 우선 반영됩니다. 그래도 찾기 어렵다면 도시명 + 장소명으로 검색해 보세요.', 'Your trip city is used first. If a place is still hard to find, search with city name + place name.')}</p></article><article><h3>{text('친구도 수정할 수 있나요?', 'Can friends edit too?')}</h3><p>{text('열람 비밀번호는 계획을 볼 수 있는 사람을, 편집 비밀번호는 수정할 수 있는 사람을 정합니다. 둘을 따로 설정해 함께 쓰면 됩니다.', 'The viewing password controls who can open a plan; the editing password controls who can change it. Set them separately when sharing.')}</p></article><article><h3>{text('실수로 계획을 삭제했어요', 'I deleted a plan by mistake')}</h3><p>{text('메인의 휴지통에서 7일 안에 복원할 수 있습니다. 7일이 지나면 자동으로 완전히 삭제됩니다.', 'Restore it from the Trash within 7 days. After that, it is permanently deleted.')}</p></article></div></section>
