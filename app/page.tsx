...
const winterCrystals = [
  [8, 14, 4, 54, 12], [31, 18, 11, -46, 10], [58, 16, 7, 62, 13], [83, 20, 15, -58, 11],
  [94, 17, 2, 48, 9],
] as const;
...
function SeasonalEffect({ season }: { season: Season['key'] }) {
  ...
  return <div className={`seasonal-effect seasonal-effect-${season}`} aria-hidden="true">
    ...
    {season === 'winter' && winterCrystals.map(([left, duration, delay, drift, size], index) => <span className="snow-crystal" key={`winter-crystal-${index}`} style={{ left: `${left}%`, animationDuration: `${duration}s`, animationDelay: `-${delay}s`, '--crystal-size': `${size}px`, '--season-drift-a': `${Math.round(drift * -.55)}px`, '--season-drift-b': `${Math.round(drift * .8)}px`, '--season-drift-c': `${Math.round(drift * -.32)}px`, '--season-drift': `${drift}px` } as CSSProperties} />)}
  </div>;
}
...
export default function HomePage() {
  ...
  return <main className={`home-landing season-${season.key} ${themeStyle}-theme`}>
    ...
    <footer className="landing-footer"><span className="landing-meta"><small>ver. 2.0</small><a href="https://github.com/whales-teatime/travel-note/releases" target="_blank" rel="noreferrer">GitHub Releases</a></span><span>{text('TRAVEL NOTE', 'TRAVEL NOTE')}</span></footer>
  </main>;
}
