    ...
    <header className="landing-topbar">
      ...
      <nav className="landing-nav" aria-label={text('주요 메뉴', 'Main menu')}>
        ...
        <div className="theme-menu-wrap">
          ...
          {themeOpen && <div className="theme-menu" role="menu"><strong>{text('배경 테마', 'Background')}</strong><button type="button" className={theme === 'auto' ? 'is-selected' : ''} onClick={() => chooseTheme('auto')}><span className="theme-swatch auto-swatch" />{text('오늘의 계절', 'Today’s season')}<small>{text('자동', 'Auto')}</small></button>{(['spring', 'summer', 'autumn', 'winter'] as const).map(key => <button type="button" className={theme === key ? 'is-selected' : ''} key={key} onClick={() => chooseTheme(key)}><span className={`theme-swatch ${key}-swatch`} />{language === 'en' ? seasonFor(key).englishLabel : seasonFor(key).label}</button>)}<div className="theme-menu-divider" /><strong>{text('언어', 'Language')}</strong><div className="language-choice" aria-label={text('언어 선택', 'Choose language')}><button type="button" className={language === 'ko' ? 'is-selected' : ''} onClick={() => setLanguage('ko')}>한국어</button><button type="button" className={language === 'en' ? 'is-selected' : ''} onClick={() => setLanguage('en')}>English</button></div><div className="theme-menu-divider" /><strong>{text('통화', 'Currency')}</strong><div className="language-choice currency-choice" aria-label={text('통화 선택', 'Choose currency')}><button type="button" className={currency === 'KRW' ? 'is-selected' : ''} onClick={() => setCurrency('KRW')}>{text('원', 'Won')}</button><button type="button" className={currency === 'USD' ? 'is-selected' : ''} onClick={() => setCurrency('USD')}>{text('달러', 'Dollars')}</button></div></div>}
        </div>
      </nav>
    </header>
