...
type EditPolicy = 'owner' | 'all';
type TripSettings = { title: string; destination: string; startDate: string; endDate: string; people: number; editPolicy: EditPolicy };
type StoredPlan = { id: string; title: string; destination: string; startDate: string; endDate: string; people: number; editPolicy?: EditPolicy; passwordProtected?: boolean; stops: Stop[] };
...
export default function Home(){
    ...
    const loadRoute=async()=>{
      if(routeId){
        try{
          ...
          const response=await fetch(`/api/plans/${encodeURIComponent(routeId)}`,{cache:'no-store',headers:token?{'x-plan-edit-token':token}:undefined}), body=await response.json() as {plan?:StoredPlan;requiresPassword?:boolean;message?:string};
          ...
          if(alive)applyStoredPlan(body.plan);
        }catch{if(alive){setPlanLoading(false);setStops([])}}
      }else if(isDraft){loadLocalDraft();if(alive)setPlanLoading(false)}
      else if(alive){setStops([]);const clean={...DEFAULT_TRIP,title:'나의 여행',destination:'',people:1};setTripSettings(clean);setSettingsDraft(clean);setPlanPassword('');setPasswordConfigured(false);setPlanPasswordTouched(false);setShowPlanPassword(false);setPlanLoading(false);if(mode==='domestic')window.setTimeout(()=>{if(alive)setSettingsOpen(true)},0)}
    };
    void loadRoute();
  ...
  const savePlan=async(silent=false)=>{
    ...
    try{
      ...
      if(body.plan)setPasswordConfigured(Boolean(body.plan.passwordProtected));
  ...
  return <main className="app-shell">
    <header className="topbar">
      ...
      <div className="top-actions">
        ...
        {planId&&<>
          ...
          <Button variant="outline" className="plan-delete-button" onClick={()=>setDeleteDialogOpen(true)} disabled={Boolean(planAction)||planLoading}><Trash2/><span>계획 삭제</span></Button>
        </>}
    ...
    <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}><DialogContent className="settings-dialog sm:max-w-[500px]"><DialogHeader><DialogTitle>여행 일정</DialogTitle><DialogDescription>어디로, 언제 떠날지 정하세요.</DialogDescription></DialogHeader><div className="trip-settings-grid"><label>여행 이름<Input value={settingsDraft.title} onChange={e=>setSettingsDraft({...settingsDraft,title:e.target.value})} placeholder="전주 맛집 여행"/></label><label>여행지<Input value={settingsDraft.destination} onChange={e=>setSettingsDraft({...settingsDraft,destination:e.target.value})} placeholder="전주"/></label><div className="date-fields"><label>출발일<Input type="date" value={settingsDraft.startDate} onChange={e=>setSettingsDraft({...settingsDraft,startDate:e.target.value})}/></label><label>돌아오는 날<Input type="date" min={settingsDraft.startDate} value={settingsDraft.endDate} onChange={e=>setSettingsDraft({...settingsDraft,endDate:e.target.value})}/></label></div><label>인원<div className="people-input"><Users/><Input type="number" min="1" max="99" value={settingsDraft.people} onChange={e=>setSettingsDraft({...settingsDraft,people:Number(e.target.value)})}/><span>명</span></div></label><label>편집 권한<select value={settingsDraft.editPolicy} onChange={e=>setSettingsDraft({...settingsDraft,editPolicy:e.target.value as EditPolicy})}><option value="owner">작성자만</option><option value="all">모두가</option></select></label><label>계획 비밀번호 <div className="password-field"><Input type={showPlanPassword?'text':'password'} value={planPassword} onChange={e=>{setPlanPassword(e.target.value);setPlanPasswordTouched(true)}} placeholder={passwordConfigured&&!planPasswordTouched?'****':'선택 입력'} aria-label="계획 비밀번호"/><button type="button" className="password-eye" onClick={()=>{if(!planPassword){setPlanSaveMessage(passwordConfigured?'현재 비밀번호는 보안상 확인할 수 없어요. 새 비밀번호를 입력하세요.':'비밀번호를 먼저 입력하세요.');return}setShowPlanPassword(current=>!current)}} aria-label={showPlanPassword?'비밀번호 숨기기':'입력한 비밀번호 보기'} title={planPassword?'입력한 비밀번호 보기':'기존 비밀번호는 확인할 수 없어요'}>{showPlanPassword?<EyeOff/>:<Eye/>}</button></div></label></div><p className="settings-hint edit-policy-hint">{settingsDraft.editPolicy==='all'?'이 계획을 열 수 있는 사람은 일정도 수정하거나 삭제할 수 있어요.':'작성자 토큰 또는 계획 비밀번호가 있어야 수정하거나 삭제할 수 있어요.'}</p><p className="settings-hint password-status">{planPasswordTouched?(planPassword?'새 비밀번호 입력됨 · 상단 계획 저장 후 적용돼요.':passwordConfigured?'저장하면 비밀번호를 해제해요.':'비밀번호 없이 저장돼요.'):(passwordConfigured?'비밀번호 설정됨 · 기존 비밀번호는 확인할 수 없어요.':'비밀번호 없음 · 선택 입력')}</p>{settingsDraft.editPolicy==='owner'&&!passwordConfigured&&!planPassword&&<div className="inline-notice owner-recovery-notice"><CircleAlert/>비밀번호 없이 작성자만을 선택하면 이 브라우저의 저장정보를 지울 때 편집 권한을 잃을 수 있어요.</div>}{passwordConfigured&&!planPasswordTouched&&<button type="button" className="password-clear" onClick={()=>{setPlanPassword('');setPlanPasswordTouched(true);setShowPlanPassword(false)}}>비밀번호 해제</button>}{!settingsDraft.destination.trim()&&<div className="inline-notice"><CircleAlert/>여행지를 입력하면 계획을 저장할 수 있어요.</div>}{planSaveMessage&&planSaveMessage!=='저장됨'&&<div className="inline-notice"><CircleAlert/>{planSaveMessage}</div>}{settingsDraft.startDate>settingsDraft.endDate&&<div className="inline-notice"><CircleAlert/>날짜를 다시 확인해주세요.</div>}<DialogFooter><Button variant="outline" onClick={()=>setSettingsOpen(false)}>취소</Button><Button onClick={saveTripSettings} disabled={!settingsDraft.startDate||!settingsDraft.endDate||settingsDraft.startDate>settingsDraft.endDate}>저장</Button></DialogFooter></DialogContent></Dialog>
  </main>
}
