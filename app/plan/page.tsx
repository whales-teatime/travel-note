...
function destinationSegments(settings:Pick<TripSettings,'destination'|'startDate'|'endDate'|'destinations'>){
  const stored=Array.isArray(settings.destinations)?settings.destinations.filter(item=>item&&typeof item==='object').map((item,index)=>({
    id:String(item.id||`destination-${index}`),name:String(item.name||''),startDate:String(item.startDate||settings.startDate),endDate:String(item.endDate||settings.endDate),...(Number.isFinite(item.lat)?{lat:Number(item.lat)}:{}),...(Number.isFinite(item.lng)?{lng:Number(item.lng)}:{}),...(item.region?{region:String(item.region)}:{}),...(item.country?{country:String(item.country)}:{}),
  })):[];
  if(stored.length)return stored;
  return settings.destination.trim()?[{id:'destination-primary',name:settings.destination.trim(),startDate:settings.startDate,endDate:settings.endDate}]:[];
}
function destinationForDay(settings:Pick<TripSettings,'destination'|'startDate'|'endDate'|'destinations'>,day:string){
  const segments=destinationSegments(settings),match=segments.find(segment=>segment.name.trim()&&segment.startDate<=day&&day<=segment.endDate);
  return match||segments.find(segment=>segment.name.trim())||null;
}
function destinationSummary(settings:Pick<TripSettings,'destination'|'startDate'|'endDate'|'destinations'>){
  const names=destinationSegments(settings).map(segment=>segment.name.trim()).filter(Boolean),seen=new Set<string>();
  return names.filter(name=>{const key=name.toLocaleLowerCase();if(seen.has(key))return false;seen.add(key);return true}).join(' · ');
}
function splitDestinationSegments(settings:Pick<TripSettings,'destination'|'startDate'|'endDate'|'destinations'>):TripDestination[]{
  const segments=destinationSegments(settings).map(segment=>({...segment}));
  const candidates=segments.map((segment,index)=>({index,count:tripDaysBetween(segment.startDate,segment.endDate).length})).filter(item=>item.count>=2).sort((left,right)=>right.count-left.count);
  if(!candidates.length)return segments;
  const target=candidates[0],segment=segments[target.index],days=tripDaysBetween(segment.startDate,segment.endDate),splitIndex=Math.max(1,Math.ceil(days.length/2));
  const first={...segment,endDate:days[splitIndex-1].date};
  const second:TripDestination={id:`destination-${Date.now().toString(36)}`,name:'',startDate:days[splitIndex].date,endDate:segment.endDate};
  segments.splice(target.index,1,first,second);
  return segments;
}
...
function EditPinTools({draft,dayKeys,text,onColorChange,onReset,onEditLocation}:{draft:Stop;dayKeys:DayKey[];text:(korean:string,english:string)=>string;onColorChange:(color:string)=>void;onReset:()=>void;onEditLocation:()=>void}){
  ...
  const defaultColor=categoryColor(draft.day,dayKeys,draft.category),[hex,setHex]=useState(draft.pinColor||defaultColor);
  useEffect(()=>setHex(draft.pinColor||defaultColor),[draft.pinColor,defaultColor]);
  useEffect(()=>{
  ...
  return createPortal(<div className="edit-pin-tools"><div className="edit-pin-color-control"><label>{text('핀 색상','Pin color')}<span><input type="color" aria-label={text('핀 색상','Pin color')} value={/^#[0-9a-f]{6}$/i.test(draft.pinColor||'')?draft.pinColor!:defaultColor} onChange={event=>{setHex(event.target.value);onColorChange(event.target.value)}}/><Input className="pin-hex-input" value={hex} onChange={event=>{const value=event.target.value;setHex(value);if(/^#[0-9a-f]{6}$/i.test(value))onColorChange(value)}} aria-label={text('색상 코드','Color hex code')} placeholder="#aaaaaa"/><code>{draft.pinColor?text('사용자 지정','Custom'):text('날짜·카테고리 기본색','Date/category default')}</code></span></label><Button type="button" variant="ghost" onClick={()=>{setHex(defaultColor);onReset()}}>{text('기본색으로','Reset')}</Button></div><Button type="button" variant="outline" onClick={onEditLocation}><MapPin/>{text('위치 임의 수정','Edit location')}</Button></div>,target);
}
...
function usePlaceSuggestions(query:string,enabled:boolean,context='',provider:MapProvider='naver',googleKey='',requestedQuery=''){
  ...
  useEffect(()=>{
    ...
    if(cached?.length){setResults(cached);setSearching(false);setError('');return}
    // Do not preserve an empty search forever. Naver can occasionally return
    // no items for one request even though the same place is available moments later.
    ...
    const timer=window.setTimeout(async()=>{
      ...
      try{
        ...
        if(items.length)suggestionCache.set(cacheKey,items);else suggestionCache.delete(cacheKey);
        setResults(items);if(!items.length)setError(text('검색 결과가 없습니다.','No results found.'));
...
function PlacePicker({query,onQueryChange,results,value,onPick,searching,placeholder,selected,onEnter,provider='naver',numbered=false}:{query:string;onQueryChange:(value:string,userInput:boolean)=>void;results:SearchPlace[];value:SearchPlace|null;onPick:(place:SearchPlace|null)=>void|Promise<void>;searching:boolean;placeholder:string;selected:boolean;onEnter?:(value:string)=>void;provider?:MapProvider;numbered?:boolean}){
  const { language } = useLanguage();
  ...
  return <Combobox<SearchPlace> items={results} filteredItems={results} filter={null} value={value} inputValue={inputValue} open={open} onOpenChange={setOpen} onInputValueChange={(next,details)=>{if(details.reason==='item-press')return;if(details.reason==='input-change'){inputValueRef.current=next;setInputValue(next);if(!composingRef.current)onQueryChange(next,true)}else if(details.reason==='input-clear'&&!suppressClearRef.current){const source=details.event as Event|undefined;const isUserDelete=Boolean(source&&'inputType' in source&&String((source as InputEvent).inputType||'').startsWith('delete'));if(isUserDelete||!inputValueRef.current.trim()){inputValueRef.current=next;setInputValue(next);onQueryChange(next,true)}}}} onValueChange={place=>{void onPick(place);if(place){const next=placeTitle(place,language);inputValueRef.current=next;setInputValue(next);setOpen(false)}}} itemToStringLabel={place=>placeTitle(place,language)}>
    <ComboboxInput className="place-combobox-input" placeholder={placeholder} showTrigger={false} showClear={Boolean(inputValue)} clearLabel={text('검색어 지우기','Clear search')} onClear={()=>{inputValueRef.current='';setInputValue('');setOpen(false);onQueryChange('',true)}} inputMode="search" enterKeyHint="search" onFocus={()=>{if(!selected&&inputValue.trim().length>=2)setOpen(true)}} onCompositionStart={()=>{composingRef.current=true}} onCompositionEnd={event=>{composingRef.current=false;const committed=event.currentTarget.value;inputValueRef.current=committed;setInputValue(committed);onQueryChange(committed,true);setOpen(committed.trim().length>=2&&!selected)}} onKeyDown={event=>{const nativeEvent=event.nativeEvent as KeyboardEvent;if(event.key==='Enter'&&!nativeEvent.isComposing&&!composingRef.current){event.preventDefault();event.stopPropagation();const current=event.currentTarget.value;suppressClearRef.current=true;inputValueRef.current=current;setInputValue(current);onQueryChange(current,false);onEnter?.(current);setOpen(current.trim().length>=2&&!selected);window.setTimeout(()=>{suppressClearRef.current=false},350)}}}/>
    <ComboboxContent className="place-combobox-content">
      ...
      <ComboboxList>{results.map((place,index)=><ComboboxItem className="place-combobox-item" key={`${place.mapx}-${place.mapy}-${index}`} value={place}>{numbered?<i className="place-result-index">{index+1}</i>:<MapPin/>}<span><strong>{placeTitle(place,language)}</strong><small>{placeCategory(place,language)}</small><em>{placeAddress(place,language)}</em></span></ComboboxItem>)}</ComboboxList>
...
function DistanceComparison({stops,dayKeys,baseIds,text,language}:{stops:Stop[];dayKeys:DayKey[];baseIds:string[];text:(korean:string,english:string)=>string;language:'ko'|'en'}) {
  ...
  return <section className="distance-comparison" aria-label={text('장소별 직선거리 비교','Straight-line distance comparison')}>
    <div className="distance-comparison-heading"><div><span><Navigation/>{text('장소별 직선거리 비교','Straight-line distance comparison')}</span><strong>{text(`${bases.length}개 거점 선택됨 · 나머지는 비교 장소`,`${bases.length} bases selected · the rest are candidates`)}</strong></div><small>{text('거점은 여행 일정의 거리 비교 설정에서 고를 수 있어요. 직선거리라 실제 이동거리와는 다를 수 있어요.','Choose bases in Trip settings. Straight-line distances may differ from actual travel distances.')}</small></div>
...
function DistanceBaseSettings({stops,dayKeys,baseIds,text,onToggleBase,canEdit}:{stops:Stop[];dayKeys:DayKey[];baseIds:string[];text:(korean:string,english:string)=>string;onToggleBase:(id:string)=>void;canEdit:boolean}) {
  return <details className="distance-base-settings">
    <summary><span><Navigation/>{text('거리 비교 거점 설정','Distance comparison bases')}</span><strong>{text(`${baseIds.length}개 선택됨`,` ${baseIds.length} selected`)}</strong></summary>
    <div className="distance-base-settings-body">
      <p>{text('기준으로 삼을 장소를 여러 개 고르세요. 선택한 장소에서 나머지 장소까지의 거리를 비교합니다.','Choose any number of base places. Distances from each base to the remaining places will be compared.')}</p>
      <div className="distance-base-picker">{stops.map((stop,index)=>{const isBase=baseIds.includes(stop.id);return <button type="button" key={stop.id} className={`distance-base-button ${isBase?'is-base':''}`} onClick={()=>onToggleBase(stop.id)} disabled={!canEdit}><b style={{background:distancePinColor(stop,dayKeys,true,isBase)}}>{index+1}</b><span><strong>{stop.name}</strong><small>{stop.day}</small></span><em>{isBase?text('거점 해제','Remove base'):text('거점 추가','Add base')}</em></button>})}</div>
...
function FuelEstimate({distanceKmValue,fuelEfficiency,fuelPrice,text,language}:{distanceKmValue:number;fuelEfficiency:number;fuelPrice:number;text:(korean:string,english:string)=>string;language:'ko'|'en'}) {
  ...
  return <section className="fuel-estimate" aria-label={text('전체 예상 유류비','Estimated trip fuel cost')}>
    <div className="fuel-estimate-heading"><div><span><span className="fuel-estimate-icon">₩</span>{text('전체 예상 유류비','Estimated trip fuel cost')}</span><small>{text(`전체 동선 · 직선거리 ${formatDistance(distanceKmValue,language)} · 도로 보정 ${ROAD_DISTANCE_FACTOR}배`,`Full route · straight-line ${formatDistance(distanceKmValue,language)} · ${ROAD_DISTANCE_FACTOR}× road factor`)}</small></div><strong>{formatFuelMoney(estimatedCost,language)}<button type="button" className="fuel-help-button" onClick={()=>setHelpOpen(current=>!current)} aria-expanded={helpOpen} aria-label={text('유류비 계산식 보기','Show fuel cost formula')} title={text('계산식 보기','Show formula')}>?</button></strong></div>
...
export default function Home(){
  ...
  const [editing,setEditing]=useState<Stop|null>(null), [editDraft,setEditDraft]=useState<Stop|null>(null), [editQuery,setEditQuery]=useState(''), [editPlaceLinked,setEditPlaceLinked]=useState(true), [editMapResults,setEditMapResults]=useState<SearchPlace[]>([]), [osmEditQuery,setOsmEditQuery]=useState('');
  const [query,setQuery]=useState(''), [picked,setPicked]=useState<SearchPlace|null>(null), [osmAddQuery,setOsmAddQuery]=useState('');
  const [mapQuery,setMapQuery]=useState(''),[mapSearchCommit,setMapSearchCommit]=useState(''),[mapPicked,setMapPicked]=useState<SearchPlace|null>(null),[mapCandidate,setMapCandidate]=useState<SearchPlace|null>(null),[mapResultPlaces,setMapResultPlaces]=useState<SearchPlace[]>([]),[osmMapQuery,setOsmMapQuery]=useState('');
  ...
  const activeDestination=destinationForDay(tripSettings,activeDay), activeDestinationName=activeDestination?.name||tripSettings.destination;
  const mapCandidateStop=useMemo<Stop|null>(()=>{
    if(!mapCandidate)return null;
    const lat=Number(mapCandidate.mapy)/1e7,lng=Number(mapCandidate.mapx)/1e7;
    if(!Number.isFinite(lat)||!Number.isFinite(lng))return null;
    return {id:'map-search-candidate',day:activeDay,time:'00:00',name:placeTitle(mapCandidate,language),category:normalizeCategory(mapCandidate.category),memo:'',address:placeAddress(mapCandidate,language),lat,lng,mapProvider:tripSettings.mapProvider,placeId:mapCandidate.placeId};
  },[activeDay,language,mapCandidate,tripSettings.mapProvider]);
  const addSuggestions=usePlaceSuggestions(query,addOpen,activeDestinationName,tripSettings.mapProvider,googleKey,osmAddQuery),editSuggestions=usePlaceSuggestions(editQuery,Boolean(editing),activeDestinationName,tripSettings.mapProvider,googleKey,osmEditQuery),mapSuggestions=usePlaceSuggestions(mapQuery,true,activeDestinationName,tripSettings.mapProvider,googleKey,osmMapQuery);
  const destinationSuggestions=useDestinationSuggestions(settingsDraft.destination,settingsOpen);
  const linkedDestinationSuggestions=useDestinationSuggestions(linkedDestinationQuery,settingsOpen&&linkedDestinationSuggestionsOpen);
  useEffect(()=>{
    if(!editing){setEditMapResults([]);return}
    if(editPlaceLinked)return;
    if(editQuery.trim().length<2){setEditMapResults([]);return}
    const candidates=editSuggestions.results.slice(0,8);
    if(tripSettings.mapProvider!=='google'){setEditMapResults(candidates);return}
    let cancelled=false;
    void Promise.all(candidates.map(place=>resolveSearchPlace(place,'google',googleKey,language).catch(()=>null))).then(items=>{
      if(!cancelled)setEditMapResults(items.filter(Boolean) as SearchPlace[]);
    });
    return()=>{cancelled=true};
  },[editing,editPlaceLinked,editQuery,editSuggestions.results,tripSettings.mapProvider,googleKey,language]);
  ...
  const updateTripRange=(field:'startDate'|'endDate',value:string)=>setSettingsDraft(current=>{
    const next={...current,[field]:value} as TripSettings;
    if(!current.destinations?.length)return next;
    const segments=destinationSegments(current).map(segment=>({...segment}));
    if(segments.length){segments[0].startDate=next.startDate;segments[segments.length-1].endDate=next.endDate;}
    return {...next,destinations:segments};
  });
  const addLinkedDestination=()=>{
    if(!settingsDraft.destination.trim()){setPlanSaveMessage(text('먼저 첫 여행지를 입력해주세요.','Enter the first destination first.'));return}
    const segments=splitDestinationSegments(settingsDraft);
    if(segments.length===destinationSegments(settingsDraft).length){setPlanSaveMessage(text('날짜가 하루뿐이라 여행지를 나눌 수 없어요.','There is only one travel day to split.'));return}
    setSettingsDraft(current=>({...current,destinations:segments}));
    const emptyIndex=segments.findIndex((segment,index)=>index>0&&!segment.name.trim());
    if(emptyIndex>=0){setLinkedDestinationIndex(emptyIndex);setLinkedDestinationQuery('');setLinkedDestinationSuggestionsOpen(false)}
  };
  const updateLinkedDestination=(index:number,field:'name'|'startDate'|'endDate',value:string)=>setSettingsDraft(current=>{
    const segments=destinationSegments(current).map(segment=>({...segment}));
    if(!segments[index])return current;
    segments[index][field]=value;
    return {...current,destinations:segments};
  });
  const removeLinkedDestination=(index:number)=>setSettingsDraft(current=>{
    const segments=destinationSegments(current).map(segment=>({...segment}));
    if(index<=0||!segments[index])return current;
    segments.splice(index,1);
    if(segments.length<=1)return {...current,destinations:undefined};
    const nextStart=segments[index]?.startDate;
    segments[index-1].endDate=nextStart?addDays(nextStart,-1):current.endDate;
    setLinkedDestinationIndex(null);setLinkedDestinationSuggestionsOpen(false);
    return {...current,destinations:segments};
  });
  const chooseLinkedDestination=(place:SearchPlace,index:number)=>{
    const value=destinationLabel(place,language),lat=Number(place.mapy)/1e7,lng=Number(place.mapx)/1e7;
    setSettingsDraft(current=>{
      const segments=destinationSegments(current).map(segment=>({...segment}));
      if(!segments[index])return current;
      segments[index]={...segments[index],name:value,...(Number.isFinite(lat)?{lat}:{}),...(Number.isFinite(lng)?{lng}:{}),...(place.region?{region:place.region}:{}),...(place.country?{country:place.country}:{})};
      return {...current,destinations:segments};
    });
  ...
  const saveTripSettings=()=>{
    ...
    if(hasLinked){
      ...
      for(let index=0;index<segments.length;index+=1){
        const segment=segments[index],validDates=/^\d{4}-\d{2}-\d{2}$/.test(segment.startDate)&&/^\d{4}-\d{2}-\d{2}$/.test(segment.endDate)&&segment.startDate<=segment.endDate;
        const contiguous=index===0?segment.startDate===next.startDate:segment.startDate===addDays(segments[index-1].endDate,1);
        if(!segment.name||!validDates||segment.startDate<next.startDate||segment.endDate>next.endDate||!contiguous){setPlanSaveMessage(text('여행지별 날짜가 이어지도록 확인해주세요.','Make sure each destination has a name and consecutive dates.'));return}
      }
      segments[segments.length-1].endDate=next.endDate;
      next.destinations=segments;
    }else next.destinations=undefined;
    if(next.editPolicy==='password'&&((!editPasswordConfigured&&!editPassword.trim())||(editPasswordTouched&&!editPassword.trim()))){setEditPasswordWarningOpen(true);return}
    const nextStart=dateDayKey(next.startDate)||firstDefaultDay;setTripSettings(next);setSettingsDraft(next);setStops(current=>current.map(stop=>({...stop,day:normalizeStoredDay(stop.day,next.startDate,next.endDate),mapProvider:stop.mapProvider||next.mapProvider})));setActiveDay(nextStart);setCustomDay(nextStart);localStorage.setItem('route-note-trip-settings',JSON.stringify(next));setDestinationSuggestionsOpen(false);setLinkedDestinationIndex(null);setLinkedDestinationSuggestionsOpen(false);setSettingsOpen(false);
  };
  ...
  const saveEdit=()=>{if(!editing||!editDraft||!isValidTime(editDraft.time))return;const updated={...editDraft,name:editDraft.name.trim()||editing.name,memo:editDraft.memo.trim(),pinColor:editDraft.pinColor&&/^#[0-9a-f]{6}$/i.test(editDraft.pinColor)?editDraft.pinColor:undefined};setStops(current=>{const next=current.map(stop=>stop.id===editing.id?updated:stop);localStorage.setItem('route-note-stops',JSON.stringify(next));return next});if(selected?.id===editing.id)setSelected(updated);closeEdit();if(planId)window.setTimeout(()=>void savePlanRef.current(),0)};
  ...
  const commitMapSearch=async(value=mapQuery)=>{const committed=value.trim();setMapSearchCommit(committed);setMapPicked(null);setMapCandidate(null);if(tripSettings.mapProvider==='naver'||tripSettings.mapProvider==='osm'){setMapResultPlaces(mapSuggestions.results.slice(0,8));return}try{const places=(await Promise.all(mapSuggestions.results.slice(0,8).map(place=>resolveSearchPlace(place,'google',googleKey,language)))).filter(Boolean) as SearchPlace[];setMapResultPlaces(places)}catch(error){setPlanSaveMessage(error instanceof Error?error.message:text('장소 검색에 실패했습니다.','Place search failed.'))}};
  useEffect(()=>{if((tripSettings.mapProvider==='naver'||tripSettings.mapProvider==='osm')&&mapSearchCommit&&mapSearchCommit===mapQuery.trim())setMapResultPlaces(mapSuggestions.results.slice(0,8))},[tripSettings.mapProvider,mapSearchCommit,mapQuery,mapSuggestions.results]);
  ...
  return <main className="app-shell">
    ...
    <Dialog open={addOpen} onOpenChange={open=>{setAddOpen(open);if(!open){setQuery('');setPicked(null)}}}>
      <DialogContent className="add-dialog sm:max-w-[540px]">
        ...
        <div className="form-grid">
          ...
          <label>{text('카테고리', 'Category')}<select value={newCategory} onChange={e=>setNewCategory(e.target.value as PlaceType)}>{PLACE_CATEGORIES.map(t=><option key={t}>{text(t, t==='식사'?'Meal':t==='간식'?'Snack':t==='관광'?'Sightseeing':t==='숙소'?'Stay':t==='교통'?'Transport':'Other')}</option>)}</select></label>
        </div>
    ...
    <Dialog open={customPinOpen} onOpenChange={open=>{if(!open)cancelCustomPin()}}>
      <DialogContent className="custom-dialog sm:max-w-[540px]">
        ...
        <div className="form-grid">
          ...
          <label>{text('카테고리', 'Category')}<select value={customCategory} onChange={e=>setCustomCategory(e.target.value as PlaceType)}>{PLACE_CATEGORIES.map(t=><option key={t}>{text(t, t==='식사'?'Meal':t==='간식'?'Snack':t==='관광'?'Sightseeing':t==='숙소'?'Stay':t==='교통'?'Transport':'Other')}</option>)}</select></label>
        </div>
    ...
    <section className={`workspace ${mapFocused?'map-focused':''} ${plannerCollapsed?'planner-collapsed':''} ${editing?'edit-map-active':''}`}>
      <aside ref={plannerPanelRef} className="planner-panel" onDragOver={updateDragAutoScroll} onWheel={handlePlannerWheel}>
        ...
        {tripSettings.viewMode==='distance'&&<DistanceComparison stops={dayStops} dayKeys={dayKeys} baseIds={distanceBaseIds} text={text} language={language}/>}
      ...
      <section className="map-panel">
        ...
        {mapCandidate&&<div className="map-place-card"><button className="map-card-close" onClick={()=>setMapCandidate(null)} aria-label={text('장소 정보 닫기','Close place info')}>×</button><span>{placeCategory(mapCandidate,language)}</span><strong>{placeTitle(mapCandidate,language)}</strong><p>{placeAddress(mapCandidate,language)}</p>{mapCandidateStop&&(tripSettings.mapProvider==='naver'?<div className="map-candidate-panorama"><div className="candidate-preview-label">{text('로드뷰','Street view')}</div><PanoramaView stop={mapCandidateStop} clientId={clientId}/></div>:<a className="candidate-panorama-link" href={googleStreetViewUrl(mapCandidateStop)} target="_blank" rel="noreferrer"><Navigation/><span><strong>{text('로드뷰 열기','Open Street View')}</strong><small>{text('새 탭에서 거리뷰를 확인할 수 있어요.','Open Street View in a new tab.')}</small></span><ExternalLink/></a>)}<div className="map-candidate-actions"><a href={placeExternalUrl(mapCandidate)} target="_blank" rel="noreferrer">{tripSettings.mapProvider==='naver'?text('네이버지도에서 상세보기','View on Naver Maps'):text('Google 지도에서 상세보기','View on Google Maps')}</a><Button onClick={prepareMapCandidate}><Plus/>{text('이 장소로 결정','Choose this place')}</Button></div></div>}
      </section>
    </section>
    ...
    <Dialog open={settingsOpen} onOpenChange={setSettingsOpen}>
      <DialogContent className="settings-dialog sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>{text('여행 일정','Trip settings')}</DialogTitle>
          <DialogDescription>{text('어디로, 언제 떠날지 정하세요.','Choose where and when you are going.')}</DialogDescription>
        </DialogHeader>
        <div className="trip-settings-grid">
          <label>{text('여행 이름','Trip name')}<Input value={settingsDraft.title} onChange={e=>setSettingsDraft({...settingsDraft,title:e.target.value})} placeholder={text('전주 맛집 여행','Jeonju food trip')}/></label>
          <label className="destination-field">
            <div className="destination-label-row"><span>{text('여행지','Destination')}</span><button type="button" className="link-destination-button" onClick={addLinkedDestination} disabled={!settingsDraft.destination.trim()}><Plus/>{text('여행지 엮기','Link a destination')}</button></div>
            <Input value={settingsDraft.destination} onChange={e=>{setSettingsDraft(current=>({...current,destination:e.target.value,destinations:current.destinations?.map((segment,index)=>index===0?{...segment,name:e.target.value}:segment)}));setDestinationSuggestionsOpen(true)}} onFocus={()=>setDestinationSuggestionsOpen(true)} onKeyDown={e=>{if(e.key==='Escape')setDestinationSuggestionsOpen(false)}} placeholder={settingsDraft.mapProvider==='google'||settingsDraft.mapProvider==='osm'?text('예: 도쿄, 파리','e.g. Tokyo, Paris'):text('전주','e.g. Jeonju')} autoComplete="off" />
            {destinationSuggestionsOpen&&(settingsDraft.destination.trim().length>=2)&&(destinationSuggestions.searching||destinationSuggestions.results.length>0)&&<div className="destination-suggestions">{destinationSuggestions.searching&&<div className="destination-suggestion-status">{text('도시를 찾는 중…','Finding cities…')}</div>}{destinationSuggestions.results.map((place,index)=><button type="button" key={`${place.placeId||place.mapx||place.title}-${index}`} className="destination-suggestion" onMouseDown={event=>event.preventDefault()} onClick={()=>chooseDestination(place)}><MapPin/><span><strong>{destinationLabel(place,language)}</strong><small>{placeAddress(place,language)}</small></span></button>)}</div>}
            {destinationSegments(settingsDraft).length>1&&<div className="destination-segment-summary"><MapPin/><span>{text('첫 여행지','First destination')} · {formatTripDate(destinationSegments(settingsDraft)[0].startDate,false,language)}–{formatTripDate(destinationSegments(settingsDraft)[0].endDate,false,language)}</span></div>}
          </label>
          {destinationSegments(settingsDraft).slice(1).map((segment,index)=>{
            const segmentIndex=index+1, isActive=linkedDestinationIndex===segmentIndex;
            return <div className="linked-destination-editor" key={segment.id||segmentIndex}>
              <div className="linked-destination-heading"><span><MapPin/>{text(`여행지 ${index+2}`,`Destination ${index+2}`)}</span><button type="button" className="linked-destination-remove" onClick={()=>removeLinkedDestination(segmentIndex)} aria-label={text('여행지 연결 해제','Remove linked destination')}>×</button></div>
              <div className="linked-destination-place">
                <Input value={segment.name} onFocus={()=>{setLinkedDestinationIndex(segmentIndex);setLinkedDestinationQuery(segment.name);setLinkedDestinationSuggestionsOpen(true)}} onChange={event=>{updateLinkedDestination(segmentIndex,'name',event.target.value);setLinkedDestinationIndex(segmentIndex);setLinkedDestinationQuery(event.target.value);setLinkedDestinationSuggestionsOpen(true)}} placeholder={text('다음 여행 도시를 입력하세요','Enter the next city')} autoComplete="off" />
                {isActive&&linkedDestinationSuggestionsOpen&&(linkedDestinationQuery.trim().length>=2)&&(linkedDestinationSuggestions.searching||linkedDestinationSuggestions.results.length>0)&&<div className="destination-suggestions linked-destination-suggestions">{linkedDestinationSuggestions.searching&&<div className="destination-suggestion-status">{text('도시를 찾는 중…','Finding cities…')}</div>}{linkedDestinationSuggestions.results.map((place,resultIndex)=><button type="button" key={`${place.placeId||place.mapx||place.title}-${resultIndex}`} className="destination-suggestion" onMouseDown={event=>event.preventDefault()} onClick={()=>chooseLinkedDestination(place,segmentIndex)}><MapPin/><span><strong>{destinationLabel(place,language)}</strong><small>{placeAddress(place,language)}</small></span></button>)}</div>}
              </div>
              <div className="date-fields linked-destination-dates"><label>{text('시작일','Start')}<Input type="date" min={settingsDraft.startDate} max={settingsDraft.endDate} value={segment.startDate} onChange={event=>updateLinkedDestination(segmentIndex,'startDate',event.target.value)}/></label><label>{text('종료일','End')}<Input type="date" min={settingsDraft.startDate} max={settingsDraft.endDate} value={segment.endDate} onChange={event=>updateLinkedDestination(segmentIndex,'endDate',event.target.value)}/></label></div>
            </div>;
          })}
          ...
          {settingsDraft.editPolicy==='password'&&<label>{text('편집 비밀번호','Editing password')}<div className="password-field"><Input type={showEditPassword?'text':'password'} value={editPassword} onChange={e=>{setEditPassword(e.target.value);setEditPasswordTouched(true)}} placeholder={editPasswordConfigured&&!editPasswordTouched?'****':text('편집 비밀번호 입력','Enter editing password')} aria-label={text('편집 비밀번호','Editing password')}/><button type="button" className="password-eye" onClick={()=>{if(!editPassword){setPlanSaveMessage(editPasswordConfigured?text('현재 편집 비밀번호는 보안상 확인할 수 없어요. 새 비밀번호를 입력하세요.','Your current editing password is hidden for security. Enter a new one to change it.'):text('편집 비밀번호를 먼저 입력하세요.','Enter an editing password first.'));return}setShowEditPassword(current=>!current)}} aria-label={showEditPassword?text('편집 비밀번호 숨기기','Hide editing password'):text('입력한 편집 비밀번호 보기','Show entered editing password')} title={editPassword?text('입력한 편집 비밀번호 보기','Show entered editing password'):text('기존 편집 비밀번호는 확인할 수 없어요','The existing password cannot be viewed')}>{showEditPassword?<EyeOff/>:<Eye/>}</button></div></label>}
          <label>{text('열람 비밀번호','Viewing password')}<div className="password-field"><Input type={showPlanPassword?'text':'password'} value={planPassword} onChange={e=>{setPlanPassword(e.target.value);setPlanPasswordTouched(true)}} placeholder={passwordConfigured&&!planPasswordTouched?'****':text('선택 입력','Optional')} aria-label={text('열람 비밀번호','Viewing password')}/><button type="button" className="password-eye" onClick={()=>{if(!planPassword){setPlanSaveMessage(passwordConfigured?text('현재 비밀번호는 보안상 확인할 수 없어요. 새 비밀번호를 입력하세요.','Your current viewing password is hidden for security. Enter a new one to change it.'):text('비밀번호를 먼저 입력하세요.','Enter a password first.'));return}setShowPlanPassword(current=>!current)}} aria-label={showPlanPassword?text('열람 비밀번호 숨기기','Hide viewing password'):text('입력한 열람 비밀번호 보기','Show entered viewing password')} title={planPassword?text('입력한 열람 비밀번호 보기','Show entered viewing password'):text('기존 비밀번호는 확인할 수 없어요','The existing password cannot be viewed')}>{showPlanPassword?<EyeOff/>:<Eye/>}</button></div></label>
        </div>
        <p className="settings-hint edit-policy-hint">{settingsDraft.editPolicy==='all'?text('열람할 수 있는 사람은 일정도 수정하거나 삭제할 수 있어요.','Anyone who can view the plan can also edit or delete it.'):settingsDraft.editPolicy==='password'?text('편집 비밀번호를 아는 사람만 일정도 수정하거나 삭제할 수 있어요.','Only people with the editing password can edit or delete the plan.'):text('작성자 토큰이 있어야 수정하거나 삭제할 수 있어요. 열람 비밀번호는 보기 전용이에요.','The author token is required to edit or delete. The viewing password is view-only.')}</p>
        <p className="settings-hint password-status">{text('편집 권한이 있으면 열람 비밀번호와 편집 비밀번호를 변경할 수 있어요.','Anyone with edit access can change both passwords.')}</p>
        {settingsDraft.editPolicy==='password'&&<p className="settings-hint password-status edit-password-status">{editPasswordTouched?(editPassword?text('새 편집 비밀번호 입력됨 · 상단 계획 저장 후 적용돼요.','New editing password entered · save the plan above to apply it.'):editPasswordConfigured?text('편집 비밀번호를 유지하려면 새 값을 입력하세요.','Enter a new value to keep the editing password.'):text('편집 비밀번호는 필수예요.','An editing password is required.')):(editPasswordConfigured?text('편집 비밀번호 설정됨 · 기존 비밀번호는 확인할 수 없어요.','Editing password set · the existing password cannot be viewed.'):text('편집 비밀번호 없음 · 입력이 필요해요.','No editing password · enter one to continue.'))}</p>}
        <p className="settings-hint password-status">{planPasswordTouched?(planPassword?text('새 열람 비밀번호 입력됨 · 상단 계획 저장 후 적용돼요.','New viewing password entered · save the plan above to apply it.'):passwordConfigured?text('저장하면 열람 비밀번호를 해제해요.','Saving will remove the viewing password.'):text('열람 비밀번호 없이 저장돼요.','The plan will be saved without a viewing password.')):(passwordConfigured?text('열람 비밀번호 설정됨 · 기존 비밀번호는 확인할 수 없어요.','Viewing password set · the existing password cannot be viewed.'):text('열람 비밀번호 없음 · 선택 입력','No viewing password · optional'))}</p>
        {settingsDraft.editPolicy==='owner'&&!passwordConfigured&&!planPassword&&<div className="inline-notice owner-recovery-notice"><CircleAlert/>{text('열람 비밀번호 없이 작성자만을 선택하면 이 브라우저의 저장정보를 지울 때 편집 권한을 잃을 수 있어요. 편집 비밀번호 설정을 사용하면 별도 비밀번호로 복구할 수 있어요.','If you choose Author only without a viewing password, clearing this browser’s data may remove your edit access. Set an editing password so you can recover access separately.')}</div>}
        {!canEdit&&planId&&<div className="inline-notice plan-settings-readonly"><CircleAlert/><span>{settingsDraft.editPolicy==='password'?text('편집 비밀번호를 입력하면 일정을 수정할 수 있어요.','Enter the editing password to make changes.'):settingsDraft.editPolicy==='all'?text('열람 비밀번호로 계획을 열면 수정할 수 있어요.','Open the plan with its viewing password to edit it.'):text('이 계획은 보기 전용으로 열려 있어 설정을 저장할 수 없어요.','This plan is view-only, so settings cannot be saved.')}</span>{settingsDraft.editPolicy==='password'&&<Button variant="outline" onClick={()=>setEditPasswordPromptOpen(true)}>{text('편집 비밀번호 입력','Enter editing password')}</Button>}</div>}
        {passwordConfigured&&!planPasswordTouched&&<button type="button" className="password-clear" onClick={()=>{setPlanPassword('');setPlanPasswordTouched(true);setShowPlanPassword(false)}}>{text('열람 비밀번호 해제','Remove viewing password')}</button>}
        {!settingsDraft.destination.trim()&&<div className="inline-notice"><CircleAlert/>{text('여행지를 입력하면 계획을 저장할 수 있어요.','Enter a destination before saving the plan.')}</div>}
        {planSaveMessage&&planSaveMessage!=='저장됨'&&<div className="inline-notice"><CircleAlert/>{planSaveMessage}</div>}
        {settingsDraft.startDate>settingsDraft.endDate&&<div className="inline-notice"><CircleAlert/>{text('날짜를 다시 확인해주세요.','Please check the dates.')}</div>}
        <DialogFooter><Button variant="outline" onClick={()=>setSettingsOpen(false)}>{text('취소','Cancel')}</Button><Button onClick={saveTripSettings} disabled={!settingsDraft.startDate||!settingsDraft.endDate||settingsDraft.startDate>settingsDraft.endDate||Boolean(planId&&!canEdit)}>{text('저장','Save')}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </main>
}
