...
export default function Home(){
  ...
  const unlockPlan=async()=>{
    ...
    try{
      ...
      applyStoredPlan(body.plan,undefined,body.canEdit);setPlanPassword(passwordPrompt);setPlanPasswordTouched(false);setPasswordPrompt('');setProtectedPlanId(null);setPasswordPromptOpen(false);
  ...
  const savePlan=async(silent=false)=>{
    ...
    try{
      ...
      const viewPasswordPayload=!existing||planPasswordTouched||(tripSettings.editPolicy==='all'&&passwordConfigured&&planPassword)?{password:planPassword}:{};
      ...
      if(body.plan){setPasswordConfigured(Boolean(body.plan.passwordProtected));setEditPasswordConfigured(Boolean(body.plan.editPasswordProtected));if(tripSettings.editPolicy==='password'&&editPassword){setEditPasswordAuth(editPassword)}else if(tripSettings.editPolicy!=='password'){setEditPasswordAuth('')}}
  ...
  const deletePlan=async()=>{
    ...
    try{
      const token=localStorage.getItem(`route-note-edit-token-${planId}`),passwordPayload=tripSettings.editPolicy==='all'&&planPassword?{password:planPassword}:{},editPasswordPayload=tripSettings.editPolicy==='password'&&editPassword?{editPassword,editPasswordAuth:editPasswordAuth||editPassword}:{};
