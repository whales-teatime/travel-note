...
function usePlaceSuggestions(query:string,enabled:boolean,context='',provider:MapProvider='naver',googleKey=''){
    ...
    const timer=window.setTimeout(async()=>{
      ...
      try{
        ...
        if(provider==='google'){
          ...
          const response=await fetch(`/api/search?q=${encodeURIComponent(value)}&near=${encodeURIComponent(context)}&lang=${language}`,{signal:controller.signal}),body=await readJsonResponse<{items?:SearchPlace[];message?:string}>(response);
          if(!response.ok)throw new Error(body.message||text('검색에 실패했습니다.','Search failed.'));
          items=(body.items||[]).map(item=>({...item,provider:'naver' as const}));
        }
  ...
  },[query,enabled,context,provider,googleKey,language,text]);
...
function PlacePicker({query,onQueryChange,results,value,onPick,searching,placeholder,selected,onEnter,provider='naver'}:{query:string;onQueryChange:(value:string,userInput:boolean)=>void;results:SearchPlace[];value:SearchPlace|null;onPick:(place:SearchPlace|null)=>void|Promise<void>;searching:boolean;placeholder:string;selected:boolean;onEnter?:(value:string)=>void;provider?:MapProvider}){
  ...
  return <Combobox<SearchPlace> items={results} filteredItems={results} filter={null} value={value} inputValue={inputValue} open={open} onOpenChange={setOpen} onInputValueChange={(next,details)=>{if(details.reason==='item-press')return;if(details.reason==='input-change'){inputValueRef.current=next;setInputValue(next);if(!composingRef.current)onQueryChange(next,true)}else if(details.reason==='input-clear'&&!suppressClearRef.current){const source=details.event as Event|undefined;const isUserDelete=Boolean(source&&'inputType' in source&&String((source as InputEvent).inputType||'').startsWith('delete'));if(isUserDelete||!inputValueRef.current.trim()){inputValueRef.current=next;setInputValue(next);onQueryChange(next,true)}}}} onValueChange={place=>{void onPick(place);if(place){const next=placeTitle(place,language);inputValueRef.current=next;setInputValue(next);setOpen(false)}}} itemToStringLabel={place=>placeTitle(place,language)}>
    ...
    <ComboboxContent className="place-combobox-content">
      <ComboboxEmpty>{searching?(provider==='google'?text('Google 지도에서 검색 중…','Searching Google Maps…'):text('네이버 지도에서 검색 중…','Searching Naver Maps…')):text('검색 결과가 없습니다.','No results found.')}</ComboboxEmpty>
...
function placeExternalUrl(place: {title:string;address:string;roadAddress:string;provider?:MapProvider;placeId?:string}) {
  return place.provider==='google'?googlePlaceUrl({name:cleanTitle(place.title),address:place.roadAddress||place.address,placeId:place.placeId}):naverPlaceUrl({name:cleanTitle(place.title),address:place.roadAddress||place.address});
}
function stopExternalUrl(stop:Stop,provider:MapProvider) { return provider==='google'||stop.mapProvider==='google'?googlePlaceUrl({name:stop.name,address:stop.address,placeId:stop.placeId,lat:stop.lat,lng:stop.lng}):naverPlaceUrl(stop) }
...
async function resolveSearchPlace(place:SearchPlace|null,provider:MapProvider,googleKey:string,language:'ko'|'en') {
  ...
  return provider==='google'?resolveGooglePlace({...place,provider:'google'},googleKey,language):{...place,provider:'naver' as const};
}
...
export default function Home(){
  ...
  const addSuggestions=usePlaceSuggestions(query,addOpen,tripSettings.destination,tripSettings.mapProvider,googleKey),editSuggestions=usePlaceSuggestions(editQuery,Boolean(editing),tripSettings.destination,tripSettings.mapProvider,googleKey),mapSuggestions=usePlaceSuggestions(mapQuery,true,tripSettings.destination,tripSettings.mapProvider,googleKey);
  ...
  const applyStoredPlan=useCallback((data:StoredPlan,editToken?:string,permission?:boolean,adminAuthenticated=false)=>{
    const mapProvider:MapProvider=data.mapProvider==='google'?'google':'naver';
  ...
  return <main className="app-shell">
    ...
    <section className={`workspace ${mapFocused?'map-focused':''} ${plannerCollapsed?'planner-collapsed':''}`}>
      ...
      <section className="map-panel">
        <div className="map-toolbar"><div className="map-toolbar-left"><Sparkles/><span><strong>DAY {Math.max(1,dayKeys.indexOf(activeDay)+1)}</strong></span></div><div className="map-toolbar-right"><button type="button" className="planner-toggle-button" onClick={()=>setPlannerCollapsed(current=>!current)} aria-expanded={!plannerCollapsed} aria-label={plannerCollapsed?text('일정 패널 펼치기','Expand planner'):text('일정 패널 접기','Collapse planner')} title={plannerCollapsed?text('일정 패널 펼치기','Expand planner'):text('일정 패널 접기','Collapse planner')}>{plannerCollapsed?<PanelLeftOpen/>:<PanelLeftClose/>}<span>{plannerCollapsed?text('일정 펼치기','Expand planner'):text('일정 접기','Collapse planner')}</span></button><span className={`naver-badge ${tripSettings.mapProvider==='google'?'google-badge':''}`}><b>{tripSettings.mapProvider==='google'?'G':'N'}</b>{tripSettings.mapProvider==='google'?'Google Maps':'NAVER Maps'}</span></div></div>
        <div className="map-place-search"><PlacePicker provider={tripSettings.mapProvider} query={mapQuery} onQueryChange={(value,userInput)=>{setMapQuery(value);if(userInput){setMapPicked(null);setMapCandidate(null);setMapResultPlaces([])}}} results={mapSuggestions.results} value={mapPicked} onPick={async place=>{try{const resolved=await resolveSearchPlace(place,tripSettings.mapProvider,googleKey,language);setMapPicked(resolved);setMapCandidate(resolved);setMapResultPlaces(resolved?[resolved]:[]);if(resolved)setMapQuery(placeTitle(resolved,language))}catch(error){setPlanSaveMessage(error instanceof Error?error.message:text('장소를 확인하지 못했습니다.','Could not load this place.'))}}} onEnter={()=>void commitMapSearch()} searching={mapSuggestions.searching} placeholder={`${tripSettings.destination} ${text('장소 검색','search places')}`} selected={Boolean(mapPicked)}/></div>
        {tripSettings.mapProvider==='google'?<GoogleMap {...mapViewProps} apiKey={googleKey}/>:<NaverMap {...mapViewProps} clientId={clientId}/>}
        {mapCandidate&&<div className="map-place-card"><button className="map-card-close" onClick={()=>setMapCandidate(null)} aria-label={text('장소 정보 닫기','Close place info')}>×</button><span>{placeCategory(mapCandidate,language)}</span><strong>{placeTitle(mapCandidate,language)}</strong><p>{placeAddress(mapCandidate,language)}</p><div><a href={placeExternalUrl(mapCandidate)} target="_blank" rel="noreferrer">{tripSettings.mapProvider==='google'?text('Google 지도에서 상세보기','View on Google Maps'):text('네이버지도에서 상세보기','View on Naver Maps')}</a><Button onClick={prepareMapCandidate}><Plus/>{text('이 장소로 결정','Choose this place')}</Button></div></div>}
      </section>
    </section>

    <Sheet open={Boolean(selected)} onOpenChange={open=>!open&&setSelected(null)}><SheetContent className="place-sheet sm:max-w-[430px]">{selected&&<><SheetHeader><div className="sheet-eyebrow"><span style={{background:dayColor(selected.day,dayKeys)}}>{stops.filter(s=>s.day===selected.day).findIndex(s=>s.id===selected.id)+1}</span>{formatTripDate(dayDates[selected.day],false,language)} · {selected.time} · {text(selected.category,selected.category==='식사'?'Meal':selected.category==='간식'?'Snack':selected.category==='관광'?'Sightseeing':selected.category==='숙소'?'Stay':'Other')}</div><SheetTitle>{selected.name}</SheetTitle><SheetDescription>{selected.address}</SheetDescription></SheetHeader><div className="sheet-body"><div className="section-title"><span>{text('거리뷰','Street view')}</span><small>{tripSettings.mapProvider==='google'?'Google Maps':text('네이버 파노라마','Naver panorama')}</small></div>{tripSettings.mapProvider==='google'?<a className="panorama-external-link" href={googleStreetViewUrl(selected)} target="_blank" rel="noreferrer"><Navigation/><span><strong>{text('Google 지도에서 스트리트뷰 열기','Open Street View in Google Maps')}</strong><small>{text('촬영된 거리뷰가 없는 곳은 일반 지도로 열릴 수 있어요.','Places without Street View coverage may open as a regular map.')}</small></span><ExternalLink/></a>:<PanoramaView stop={selected} clientId={clientId}/>} {selected.memo&&<div className="place-note"><span>{text('메모','Note')}</span><p>{selected.memo}</p></div>}<a className={`naver-link ${tripSettings.mapProvider==='google'?'google-link':''}`} href={stopExternalUrl(selected,tripSettings.mapProvider)} target="_blank" rel="noreferrer"><span><b>{tripSettings.mapProvider==='google'?'G':'N'}</b>{tripSettings.mapProvider==='google'?text('Google 지도에서 상세보기','View on Google Maps'):text('네이버지도에서 상세보기','View on Naver Maps')}</span><ExternalLink/></a><Button variant="outline" className="location-edit-button" onClick={startLocationEdit}><MapPin/>{text('위치 임의 수정','Edit location')}</Button><Button variant="destructive" className="delete-button" onClick={removeSelected}><Trash2/>{text('이 장소 삭제','Delete place')}</Button></div></>}</SheetContent></Sheet>
