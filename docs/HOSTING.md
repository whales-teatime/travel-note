...
- FTS 전체 내보내기가 실패하는 경우에도 `plans`, `feedback`, `feedback_comments`를 모두 포함하는 `npm run backup:d1` 대체 백업을 사용한다. 검색 색인과 장소 검색 캐시는 파생 데이터라 배포 때 다시 만들 수 있지만, 댓글은 원본 데이터라 누락하면 안 된다.
- 동시 저장 충돌본은 공개 제목에 접속 IP를 넣지 않고 시간과 짧은 임의 식별자로 구분한다.
